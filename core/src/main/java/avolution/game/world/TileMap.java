...
public class TileMap implements Disposable {
    private static final int MAP_SIZE = 0b10000001;
    ...
    private void initFoods() {
        DiamondSquareGenerator<Tile> generator = new DiamondSquareGenerator<>(Tile::updateFood, tile -> (int) tile.food());
        generator.generate(tiles, 100, RANDOM);
    }

    private void initWater() {
        Map<Tile, Integer> values = new HashMap<>();
        DiamondSquareGenerator<Tile> generator = new DiamondSquareGenerator<>((tile, waterValue) -> {
            if (waterValue > 10) {
                tile.makeWater();
            }
            values.put(tile, waterValue);
        }, values::get);
        generator.generate(tiles, 50, RANDOM);
    }
